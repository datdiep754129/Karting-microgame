...
public class BeltSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < cubeDensity; i++)
        {
            ...
            _asteroid.AddComponent<BeltObject>().SetupBeltObject(Random.Range(minOrbitSpeed, maxOrbitSpeed), Random.Range(minOrbitSpeed, maxOrbitSpeed), gameObject, rotatingClockwise);
